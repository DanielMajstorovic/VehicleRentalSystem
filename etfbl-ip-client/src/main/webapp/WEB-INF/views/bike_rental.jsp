...
<body class="client-body">
	...
	<div class="container py-3">
		<h4 class="mb-3">Select an e‑bike</h4>

		<div class="d-flex flex-column gap-3">
			<c:forEach items="${bikes}" var="b">
				<div class="card bike-card" data-vehicle-id="${b.id}"
					onclick="chooseBike(this, '${b.uid}', ${b.id}, ${b.pricePerSecond})">
					<img class="bike-img"
						src="http://localhost:8080/images/vehicles/${b.uid}.jpg"
						onerror="
              this.onerror = null;
              this.src = '${pageContext.request.contextPath}/assets/images/bike.png';
            " />
					<div class="card-body py-2">
						<div class="d-flex justify-content-between">
							<span> <strong>${b.manufacturerName}</strong> ${b.model}
							</span> <span class="text-primary small"> <c:set
									var="hourlyPrice" value="${b.pricePerSecond * 3600}" /> $<fmt:formatNumber
									value="${hourlyPrice}" type="number" minFractionDigits="2"
									maxFractionDigits="2" /> /hour
							</span>
						</div>
					</div>
				</div>
			...
			<c:if test="${empty bikes}">
				<div class="alert alert-info text-center">No e‑bikes
					available.</div>
			</c:if>
		</div>

		<form id="rentForm" class="mt-4 d-none" action="ebike" method="post">
    ...
    function chooseBike(el, uid, id, pps) {
    ...
    function initMap() {
      ...
      const setLatLng = latlng => {
        startX.value = latlng.lng.toFixed(6);
        startY.value = latlng.lat.toFixed(6);
      };
